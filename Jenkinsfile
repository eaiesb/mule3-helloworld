...
steps {
          ...
          slackSend (color: "add8e6", message: 'calculator Deployment Started')
          buildsrc() 
          slackSend (color: "0000ff", message: 'calculator Build Sucessfully')
...
stage('upload to atifactory') {
  ...
  "files": [
    {
      "pattern": "**/*.jar",
      "target": "generic-local/mule4-demo-jars_$BUILD_NUMBER/calculator.jar"
    }
 ]
}"""                 
   ...
   post {
      failure {
            emailext attachLog: true, body: 'Deployment has failed', subject: 'Mule 4 Cloud Deployment Status', to: 'sudheekarreddy.donapati@eaiesb.com'
           slackSend (color: "#FF0000",message: 'calculator Deployment Failed')
        }
      success {
